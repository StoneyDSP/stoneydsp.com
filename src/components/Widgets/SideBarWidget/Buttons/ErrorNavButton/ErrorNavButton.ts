...
class ErrorNavButton<
  Spec extends ErrorNavButton.TemplateSpec = ErrorNavButton.TemplateSpec,
  Config extends ErrorNavButton.TypeConfig = ErrorNavButton.TypeConfig,
>
  extends ButtonComponent<Spec, Config>
  implements ButtonComponent.ImplementTemplateSpec<ErrorNavButton.TemplateSpec>
{
  static override _template(): ErrorNavButton.Template<ErrorNavButton.TemplateSpec> {
    return {
      ...super._template(),
      rect: true,
      color: 0x00000000,
  ...
  protected override _onClick() {
    (this as ErrorNavButton).signal("onPress");
    Router.navigate("!");
  }
}

declare namespace ErrorNavButton {
