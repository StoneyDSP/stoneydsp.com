...
/**
 * Toggle endpoint backed by JUCE WebToggleRelay state
 */
export function juceToggleEndpoint(
  name: string,
  emit: (v: boolean) => void
): ControlEndpoint {
  const t: JUCE.ToggleState = JUCE.getToggleState(name);
...
/**
 * Slider endpoint backed by JUCE WebSliderRelay state (normalised 0..1)
 */
export function juceSliderEndpoint(
  name: string,
  emit: (v01: number) => void
): ControlEndpoint {
  const s: JUCE.SliderState = JUCE.getSliderState(name);
...
/**
 * Combo endpoint backed by JUCE WebComboBoxRelay state.
 * Value is the selected choice *string* (we map to/from choice index).
 */
export function juceComboEndpoint(
  name: string,
  emit: (choice: string) => void
): ControlEndpoint {
  const c: JUCE.ComboBoxState = JUCE.getComboBoxState(name);
