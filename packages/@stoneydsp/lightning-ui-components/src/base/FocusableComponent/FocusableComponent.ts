...
abstract class FocusableComponent<
  ...
  Config extends FocusableComponent.TypeConfig =
    FocusableComponent.TypeConfig,
>
  extends BaseComponent<Spec, Config>
  implements
    BaseComponent.ImplementTemplateSpec<FocusableComponent.TemplateSpec>
{
  ...
  protected override _data: FocusableComponent.Data;

  override get data() {
    return this._data;
  }

  override set data(data) {
    this._data = data;
  }
  ...
  override _handleHover(target: Parameters<NonNullable<BaseComponent['_handleHover']>>[0]) {
    if (super._handleHover) return super._handleHover(target);
  }

  override _handleUnhover(target: Parameters<NonNullable<BaseComponent['_handleUnhover']>>[0]) {
